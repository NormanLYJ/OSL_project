...
class ReactSigmaGraph extends React.Component {
  ...
  componentWillReceiveProps(nextProps){
    ...
    if ( this.didDataChange(nextProps.data, this.getData())){
      this.drawGraph();
    }
    // this.drawGraph();
  }
  ...
  didDataChange(prevData, newData) {
    ...
    let areNodesEqual = deepEqual(prevData.nodes,newData.nodes);
    let areEdgesEqual = deepEqual(prevData.edges, newData.edges)
    if (prevData.edges.length==0){
...
const DEFAULT_Y = 0;
const N_TICKS = 350;
const EDGE_COLOR = '#192531';
const HIGHLIGHTED_EDGE_COLOR = '#4576a7';
